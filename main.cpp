...
reverse(all(codeWord));
cout<<"Encoded codeWord is: "<<codeWord;e
cout<<"*******************\n";

string recivedCodeWord; cin>>recivedCodeWord;
vll res_of_Ps_InResivedCodeWord(r);
x=1;
reverse(all(recivedCodeWord));
...
int error=0;
// cout<<res_of_Ps_InResivedCodeWord;e
fr(i,r){
...
if(error==0&&recivedCodeWord==codeWord){
    cout<<"no error";e
    cout<<"codeWord = "<<recivedCodeWord;e
}
else {
    if(error==0){
     fr(i,n){
